...
task :source_user_infom => :environment  do
	tmp = []                                              #task shenyang
	CSV.open("syimport20120629.csv", 'wb') do |csv|
		CSV.foreach("#{Rails.root}/lib/data/2012_icolor_user/shenyangcom20120629.csv") do |row|
			puts "starting................"
	...
	CSV.open("syfailed_tasks.csv", 'wb') do |csv|
		tmp.each {|f| csv << [f]}
	end

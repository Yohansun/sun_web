...
class WeiboItem < ActiveRecord::Base
  # attr_accessible :title, :body
  validates_uniqueness_of :idstr

  def self.fetch(page = 1)
    weibo_keywords = MagicSetting.weibo_keywords || '放飞色彩心愿清单'
    weibo_keywords.split(",").each do |q|
      ...
      lines['statuses'].each do |status|
        WeiboItem.create(:q => q, :idstr => status['idstr'], :text => status['text'],

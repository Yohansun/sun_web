...
class BaichengEvent < ActiveRecord::Base
  ...
  def self.story_type(area_id)
    ...
    BaichengEvent.where("area_id in (#{area_arr_id}) and eventable_type = 'Story'").order('id desc').limit(6)
  end

  def self.design_type(area_id)
    area_arr_id = Area.where(parent_id: area_id).map &:id
    area_arr_id = area_arr_id.join(",")
    BaichengEvent.where("area_id in (#{area_arr_id}) and eventable_type = 'Design'").order('id desc').limit(6)
  end

  def self.baicheng_map_design(area_id)
    area_arr_id = Area.where(parent_id: area_id).map &:id
    area_arr_id = area_arr_id.join(",")
    BaichengEvent.where("area_id in (#{area_arr_id}) and eventable_type = 'Design'").order('id desc').limit(12)
  end

  def self.baicheng_map_story(area_id)
    area_arr_id = Area.where(parent_id: area_id).map &:id
    area_arr_id = area_arr_id.join(",")
    BaichengEvent.where("area_id in (#{area_arr_id}) and eventable_type = 'Story'").order('id desc').limit(12)
  end

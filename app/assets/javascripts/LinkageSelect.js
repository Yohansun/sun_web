...
function LinkageSelect(options) {
	 ...
	 * @param {Object} value
	 */
	function _bind(element , value, prompt) {
		...
		for(var el_id in bindEls){
			...
			if (el_id < self_id){
				bindEls[el_id].element.change(function() {
					_fill(element, undefined, undefined, prompt);
				})
			}
		}
		...
		if (self_id > 0) {
			bindEls[self_id-1].element.change(function() {
				_fill(element , bindEls[self_id].key, undefined, prompt);
			});
		}
				...
				if (typeof bindEls[self_id+1] == 'undefined') {
					_find(self_key , function(key , json) {
						if (json) {
							...
							element.after(el);
							_bind(el , '');
							_fill(bindEls[self_id+1].element , key , json, prompt);
						}
					});
				}
			}
		})
		_fill(element , key , value, prompt);
	 ...
	 * @param {Object} value
	 */
	function _fill(element , key , value, prompt) {

		element.empty();
		element.append('<option value="">' + prompt + '</option>');
	...
	return {
		...
		// 绑定元素
		bind	: function(element , value, prompt) {

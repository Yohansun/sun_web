function ImageFlow()
{this.defaults={animationSpeed:50,aspectRatio:2.5,buttons:false,captions:0,circular:false,imageCursor:'default',ImageFlowID:'imageflow',imageFocusM:1.0,imageFocusMax:2,imagePath:'',imageScaling:true,imagesHeight:0.67,imagesM:0.5,onClick:function(){document.location=this.url;},opacity:true,opacityArray:[10,3,1,1,1],percentLandscape:100,percentOther:100,preloadImages:true,reflections:0,reflectionGET:'',reflectionP:0.5,reflectionPNG:false,reflectPath:'',scrollbarP:0.6,slider:0,sliderCursor:'e-resize',sliderWidth:14,slideshow:false,slideshowSpeed:1500,slideshowAutoplay:false,startID:1,glideToStartID:true,startAnimation:false,xStep:80};var my=this;this.init=function(options)
...
{window.onload=run;}}};var domReady=function(handler){domReadyEvent.add(handler);};domReadyEvent.init();domReady(function()
{var instanceOne=new ImageFlow();instanceOne.init({ImageFlowID:'gallery'});var $gallery=$('#gallery');$gallery.parent().find('span.moveleft').click(function(){instanceOne.MouseWheel.handle(1);});$gallery.parent().find('span.moveright').click(function(){instanceOne.MouseWheel.handle(-1);});my.MouseWheel.handle});
...
class Manage::BannerChannelController < Manage::BaseController
	...
	def index
  	...
  	if params[:position]
  		@i_banner = IBanner.page_name('业主通道').position(params[:position]).first
  	else
  		@i_banner = IBanner.page_name('业主通道').first
  	end
  end

  def create_image
  	position =  params[:position]
    @i_banner = IBanner.page_name('业主通道').position(position).first
    result = "上传失败"
    if @i_banner
  		@i_banner.file = params[:file].tempfile if params[:file].present?
  		@i_banner.page_name = '业主通道'
    	if @i_banner.save
